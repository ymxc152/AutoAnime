...
const KIND_LABELS: Record<string, string> = { source: '下载源', library: '媒体库', operations: '操作日志', execute: '整理', correction: '纠正', interval: '间隔', daily: '每天', backup: '备份', manual_rollback: '回滚' }
const JOB_TYPE_LABELS: Record<string, string> = { scan: '扫描', execute_plan: '整理执行', rollback_operation: '回滚' }
const REVIEW_TYPE_LABELS: Record<string, string> = { low_confidence: '置信度不足', conflict: '路径冲突', identity: '识别待确认' }
const STAGE_LABELS: Record<string, string> = { scan: '扫描', identify: '识别', execute: '整理', rollback: '回滚', discover: '发现文件' }
const EVENT_TYPE_LABELS: Record<string, string> = { phase: '阶段', identify_started: '开始识别', identify_unit: '识别中', scan_completed: '扫描完成', execution_completed: '整理完成', rollback_completed: '回滚完成', progress: '进度' }
const ACTION_LABELS: Record<string, string> = { link: '硬链接', copy: '复制', move: '移动', skip: '跳过', conflict: '冲突', organize: '整理' }
const TITLE_REASON_PRESETS = ['识别出错', '译名不统一', '合并重复条目', '作品名写错'] as const
...
export function ProfilesPage() {
  ...
  const [kind, setKind] = useState('source'); const [path, setPath] = useState(''); const [creating, setCreating] = useState(false); const [editing, setEditing] = useState<Item | null>(null); const [pathError, setPathError] = useState(''); const [actionError, setActionError] = useState(''); const [scanSuccess, setScanSuccess] = useState(false)
  ...
  const addProfile = useMutation({ mutationFn: (profile: Item) => api.post('/profiles', profile), onSuccess: async () => { setActionError(''); closeEditor(); await client.invalidateQueries({ queryKey: ['profiles'] }) }, onError: reason => setActionError(reason instanceof Error ? reason.message : '创建扫描方案失败') })
  const updateProfile = useMutation({ mutationFn: ({ profile, patch }: { profile: Item; patch: Item }) => api.patch(`/profiles/${profile.id}`, { revision: profile.revision, patch }), onSuccess: async () => { setActionError(''); closeEditor(); await client.invalidateQueries({ queryKey: ['profiles'] }) }, onError: reason => setActionError(reason instanceof Error ? reason.message : '保存失败') })
  const toggleProfile = useMutation({ mutationFn: (profile: Item) => api.patch(`/profiles/${profile.id}`, { revision: profile.revision, patch: { enabled: !Boolean(profile.enabled) } }), onSuccess: async () => { setActionError(''); await client.invalidateQueries({ queryKey: ['profiles'] }) }, onError: reason => setActionError(reason instanceof Error ? reason.message : '切换失败') })
  const deleteProfile = useMutation({ mutationFn: (profile: Item) => api.delete(`/profiles/${profile.id}`, { revision: profile.revision }), onSuccess: async () => { setActionError(''); closeEditor(); await client.invalidateQueries({ queryKey: ['profiles'] }) }, onError: reason => setActionError(reason instanceof Error ? reason.message : '删除失败') })
  ...
  return <Page title="目录与扫描" description="先选择下载目录和媒体库，再创建一个扫描方案。" actions={<button className="secondary" onClick={() => roots.refetch()}><RefreshCw size={16} />刷新状态</button>}>
    ...
    <div className="split"><section className="surface"><div className="surface-title"><h2>目录</h2><span>本机可点“浏览”选择文件夹</span></div><form className="inline-form" onSubmit={event => { event.preventDefault(); addRoot.mutate() }}><select aria-label="目录类型" value={kind} onChange={event => setKind(event.target.value)}><option value="source">下载源</option><option value="library">媒体库</option><option value="operations">操作日志（更多）</option></select><input aria-label="目录路径" value={path} onChange={event => setPath(event.target.value)} placeholder="F:\动漫下载" /><button type="button" className="secondary" onClick={() => pickFolder.mutate()} disabled={pickFolder.isPending}>{pickFolder.isPending ? '选择中…' : '浏览…'}</button><button className="primary" disabled={!path}><Plus size={16} />添加</button></form>{pathError ? <div className="form-error form-indent">{pathError}</div> : null}<p className="muted form-indent">局域网浏览器无法弹出服务器本机对话框，请直接粘贴路径。可添加多个下载源和多个媒体库；每个扫描方案选一对；多个方案可以指向同一媒体库。</p><DataTable items={roots.data?.items || []} columns={['kind', 'path', 'health_status', 'enabled']} action={root => { const isReferenced = Number(root.profile_count || 0) > 0 || Number(root.file_count || 0) > 0; return <div className="row-actions"><button className="text-button" onClick={() => validateRoot.mutate(root.id)}>验证</button><button className="text-button" onClick={() => toggleRoot.mutate(root)}>{root.enabled ? '停用' : '启用'}</button>{!isReferenced ? <button className="text-button danger" onClick={() => { if (window.confirm(`删除目录“${root.path}”？`)) deleteRoot.mutate(root) }}><Trash2 size={13} />删除</button> : null}</div> }} /></section>
      <section className="surface"><div className="surface-title"><h2>扫描方案</h2><button type="button" className="secondary" onClick={() => { setActionError(''); setEditing(null); setCreating(true) }}><Plus size={16} />新建扫描方案</button></div>{profiles.error ? <Empty title="无法读取扫描方案" description={queryErrorMessage(profiles.error, '请稍后重试')} /> : profiles.data?.items.length ? profiles.data.items.map(profile => <div className={editing?.id === profile.id ? 'profile-row active' : 'profile-row'} key={profile.id}><div><strong title={profile.name}>{profile.name}</strong><span>{MODE_LABELS[String(profile.mode)] || profile.mode} · {POLICY_LABELS[String(profile.execution_policy)] || profile.execution_policy} · 阈值 {profile.min_confidence}%</span><span>{rootPath(profile.source_root_id)} → {rootPath(profile.library_root_id)}</span></div><div className="row-actions"><button className="text-button" onClick={() => { setActionError(''); setCreating(false); setEditing(profile) }}>编辑</button><button className="secondary" onClick={() => scan.mutate(profile.id)} disabled={!profile.enabled}><ScanSearch size={16} />手动扫描</button><button className="text-button" onClick={() => toggleProfile.mutate(profile)}>{profile.enabled ? '停用' : '启用'}</button><button className="text-button danger" onClick={() => { if (window.confirm(`删除扫描方案“${profile.name}”？方案会从当前配置中消失，但扫描记录、整理计划和回滚审计会保留。`)) deleteProfile.mutate(profile) }}><Trash2 size={13} />删除</button></div></div>) : <Empty title="还没有扫描方案" description="添加下载源和媒体库后，点右上角「新建扫描方案」。" />}<p className="muted form-indent">建议先用“全部审核”。开启 AI 可减少待确认项（设置 → 常用 → AI 识别）</p></section></div>
    <Dialog open={creating || Boolean(editing)} title={editing ? '编辑扫描方案' : '新建扫描方案'} description={editing ? '修改后点击「保存配置」生效。' : '选择一对下载源和媒体库，建议先用全部审核。'} onClose={closeEditor} size="lg">
      {actionError ? <div className="form-error">{actionError}</div> : null}
      {editing ? <ProfileForm key={editing.id} editing initial={editing} roots={roots.data?.items || []} onCancel={closeEditor} onSave={patch => updateProfile.mutate({ profile: editing, patch: { name: patch.name, source_root_id: patch.source_root_id, library_root_id: patch.library_root_id, mode: patch.mode, execution_policy: patch.execution_policy, min_confidence: patch.min_confidence, stability_seconds: patch.stability_seconds, watch_enabled: patch.watch_enabled, enabled: patch.enabled } })} /> : creating ? <ProfileForm key="new" initial={defaultProfile} roots={roots.data?.items || []} onCancel={closeEditor} onSave={profile => addProfile.mutate(profile)} /> : null}
    </Dialog>
  </Page>
}
function JobsPanel({ active = true }: { active?: boolean }) {
  ...
  return <div className="plan-layout review-layout">{jobError ? <div className="form-error">{jobError}</div> : null}<section className="surface"><DataTable items={query.data.items} columns={['id', 'job_type', 'current_stage', 'progress_current', 'status', 'created_at']} onRow={setSelected} selectedId={selected?.id} action={item => ['queued', 'leased', 'running'].includes(item.status) ? <button className="text-button" onClick={() => cancel.mutate(item.id)}>安全取消</button> : null} /></section><aside className="inspector">{selected ? <><h2>任务 #{selected.id}</h2><Status value={selected.status} /><dl><dt>类型</dt><dd>{JOB_TYPE_LABELS[String(selected.job_type)] || selected.job_type}</dd><dt>阶段</dt><dd>{STAGE_LABELS[String(selected.current_stage)] || selected.current_stage || '—'}</dd><dt>错误</dt><dd>{selected.error_summary ? translateApiMessage(String(selected.error_code || 'http_error'), String(selected.error_summary)) : '—'}</dd></dl><AgentRail events={events.data || []} memoryCount={memory.data?.items.length || 0} recentMemory={(memory.data?.items || []).slice(0, 6).map(item => ({ alias_key: String(item.alias_key || ''), canonical_title: String(item.canonical_title || ''), source: String(item.source || '') }))} /><h3>事件记录</h3><div className="event-list">{events.data?.length ? events.data.map(event => <div key={event.sequence}><strong>#{event.sequence} {EVENT_TYPE_LABELS[event.type] || event.type}</strong><span>{event.message || JSON.stringify(event.payload)}</span></div>) : <Empty>暂无事件</Empty>}</div></> : <Empty>选择任务查看事件</Empty>}</aside></div>
}
...
const AGENT_LABELS: Record<string, string> = { filename: '文件名', parser: '解析器', aiparse: 'AI 识别', metadata: '元数据', memory: '已记住别名', bangumi: 'Bangumi', tmdb: 'TMDB', bgm: 'Bangumi' }
...
function candidateLabel(cand: Item): string {
  ...
  if (cand.source === 'metadata') {
    const provider = cand.provider === 'bgm' || cand.provider === 'bangumi' ? 'Bangumi' : cand.provider === 'tmdb' ? 'TMDB' : String(cand.provider || '')
...
export function ReviewResolutionForm({ review, onSubmit, submitting = false }: { review: Item; onSubmit: (resolution: Item) => void; submitting?: boolean }) {
  ...
  return <form className="review-resolution-form" onSubmit={submit}>
    ...
    <h3>识别依据</h3>
    <div className="evidence-list">{evidenceList.length ? evidenceList.map((item, index) => <div className="evidence-item" key={index}><span className="evidence-agent">{AGENT_LABELS[String(item.agent || '')] || String(item.agent || '')}</span><span className="evidence-value">{String(item.value ?? '')}</span><span className="evidence-conf">{item.confidence !== undefined ? `${Math.round(Number(item.confidence) * 100)}%` : ''}</span></div>) : <p className="muted">无识别证据</p>}</div>
...
export function LibraryPage() {
  const client = useQueryClient(); const [selected, setSelected] = useState<Item | null>(null); const [title, setTitle] = useState(''); const [reasonChoice, setReasonChoice] = useState<string>(TITLE_REASON_PRESETS[0]); const [reason, setReason] = useState(''); const [locked, setLocked] = useState(true); const [preview, setPreview] = useState<Item | null>(null); const [libError, setLibError] = useState('')
  const resolvedReason = reasonChoice === '自定义' ? reason.trim() : reasonChoice
  const [q, setQ] = useState(''); const [debouncedQ, setDebouncedQ] = useState(''); const [sort, setSort] = useState('title'); const [correcting, setCorrecting] = useState(false)
  ...
  const previewChange = useMutation({ mutationFn: () => api.post<Item>('/library/changes/preview', { show_id: selected?.id, base_revision: selected?.revision, patch: { canonical_title: title, title_locked: locked }, reason: resolvedReason }), onSuccess: setPreview, onError: reason => setLibError(reason instanceof Error ? reason.message : '预览修改失败') })
  const approve = useMutation({ mutationFn: () => api.post<Item>(`/library/changes/${preview?.id}/approve`), onSuccess: async updated => { const showId = Number(updated?.id ?? selected?.id); setPreview(null); setTitle(''); setReasonChoice(TITLE_REASON_PRESETS[0]); setReason(''); setCorrecting(false); await client.invalidateQueries({ queryKey: ['shows'] }); if (showId) { await client.invalidateQueries({ queryKey: ['show', showId] }); const refreshed = await api.get<Item>(`/library/shows/${showId}`).catch(() => updated); setSelected(refreshed) } }, onError: reason => setLibError(reason instanceof Error ? reason.message : '批准修改失败') })
  const metadata = detail.data?.metadata?.[0]
  const moving = Number(impact.data?.files_to_move ?? 0); const discarding = Number(impact.data?.files_to_discard ?? 0); const missing = Number(impact.data?.files_missing ?? 0)
  ...
  return <Page title="资料库" description="查看番剧、季集、文件位置、识别证据和附加元数据，搜索并纠正整理结果">
    {query.error ? <Empty title="无法读取资料库" description={queryErrorMessage(query.error, '请稍后重试')} /> : !query.data?.items.length && !debouncedQ ? <Empty title="资料库还是空的" description="批准并完成一次整理后，番剧会显示在这里。" cta={{ label: '配置目录', to: '/scan' }} /> : <div className="plan-layout library-layout"><section className="surface">
      ...
      {query.data?.items.length ? <div className="library-list">{query.data.items.map(item => <button type="button" key={item.id} className={`library-row${selected?.id === item.id ? ' active' : ''}`} onClick={() => { setSelected(item); setTitle(item.canonical_title); setPreview(null); setCorrecting(false); setReasonChoice(TITLE_REASON_PRESETS[0]); setReason('') }}><span className="library-row-title"><strong>{item.canonical_title}</strong>{recentWithin(item.created_at, 14) ? <em className="badge-new">新番</em> : null}{recentWithin(item.recent_activity, 14) ? <em className="badge-updated">更新</em> : null}</span><span className="library-row-meta">{item.season_count ?? 0} 季 · {item.episode_count ?? 0} 集</span><span className="library-row-time">{shortDate(item.recent_activity)}</span></button>)}</div> : <Empty title="没有匹配的番剧" description="换个关键词再试试。" />}
    </section><aside className="inspector">{selected ? <>{metadata?.poster_url ? <img className="poster" src={metadata.poster_url} alt={`${selected.canonical_title} 海报`} /> : null}<h2>{selected.canonical_title}</h2><p className="muted-copy">{metadata?.synopsis || '暂无简介；元数据不可用不会影响文件整理。'}</p><dl><dt>放送状态</dt><dd>{metadata?.broadcast_status || '未知'}</dd><dt>季数</dt><dd>{detail.data?.seasons?.length ?? 0}</dd><dt>修订</dt><dd>{selected.revision}</dd><dt>最近更新</dt><dd>{shortDate(selected.recent_activity)}</dd></dl><div className="season-blocks">{detail.data?.seasons?.map((season: Item) => <div className="season-block" key={season.id}><div className="season-head"><strong>{season.season_number === 0 ? '特别篇' : `Season ${String(season.season_number).padStart(2, '0')}`}</strong><span>{season.episodes?.length ?? 0} 集</span></div>{season.episodes?.length ? <div className="episode-list">{season.episodes.map((ep: Item) => { const files = Array.isArray(ep.files) && ep.files.length ? ep.files : [null]; return files.map((file: Item | null, index: number) => <div className="episode-item" key={`${ep.id}-${index}`}><span className="ep-token">{index === 0 ? episodeLabel(ep) : ''}</span><span className="ep-file" title={file?.path || ''}>{file?.path ? fileBase(file.path) : (ep.display_title || '—')}</span>{file?.release_label ? <span className="ep-tag">{file.release_label}</span> : null}</div>) })}</div> : <p className="muted">暂无剧集</p>}</div>)}</div><div className="form-actions"><button type="button" className="secondary" onClick={() => { setCorrecting(true); setPreview(null); setLibError(''); setReasonChoice(TITLE_REASON_PRESETS[0]); setReason('') }}>纠正标题</button></div><ReviewChat kind="library" targetId={Number(selected.id)} onApplied={async () => { const showId = Number(selected.id); await client.invalidateQueries({ queryKey: ['shows'] }); await client.invalidateQueries({ queryKey: ['show', showId] }); const refreshed = await api.get<Item>(`/library/shows/${showId}`).catch(() => selected); setSelected(refreshed); setTitle(String(refreshed.canonical_title || title)); setPreview(null); setCorrecting(false) }} /><Dialog open={correcting} title="纠正标题" description={`正在修改「${selected.canonical_title}」`} onClose={() => setCorrecting(false)} size="lg"><label>新规范标题<input aria-label="新规范标题" value={title} onChange={event => setTitle(event.target.value)} /></label><label>修改原因<div className="chip-row" role="group" aria-label="修改原因">{TITLE_REASON_PRESETS.map(item => <button type="button" key={item} className={`chip${reasonChoice === item ? ' active' : ''}`} onClick={() => { setReasonChoice(item); setPreview(null) }}>{item}</button>)}<button type="button" className={`chip${reasonChoice === '自定义' ? ' active' : ''}`} onClick={() => { setReasonChoice('自定义'); setPreview(null) }}>自定义</button></div>{reasonChoice === '自定义' ? <input aria-label="自定义修改原因" value={reason} onChange={event => setReason(event.target.value)} placeholder="说明为什么改这个标题" /> : null}</label><label className="check-field"><input type="checkbox" checked={locked} onChange={event => setLocked(event.target.checked)} />锁定人工标题</label>{libError ? <div className="form-error">{libError}</div> : null}{missing > 0 ? <div className="form-error">媒体库里有 {missing} 个文件已不在记录路径上（可能是硬链接源被删除后链接失效）。请先把文件放回原位，或重新扫描整理后再纠正标题。</div> : null}{impact.data?.merge ? <div className="risk-warning">该标题已是番剧「{impact.data.target_show?.canonical_title}」。批准后将合并：移动 {moving} 个文件，同集自动留大删小（{discarding} 个）。</div> : moving + discarding > 0 && missing === 0 ? <div className="risk-warning">批准后将移动 {moving} 个文件{discarding > 0 ? `，并删小留大 ${discarding} 个` : ''}到新文件夹。</div> : null}{preview && missing === 0 ? <div className="change-preview"><strong>修改预览</strong><span>{String(preview.old_values?.canonical_title)} → {String(preview.new_values?.canonical_title)}</span><button className="primary full" onClick={() => approve.mutate()}><ShieldCheck size={16} />批准修改</button></div> : <button className="secondary full" disabled={!title || !resolvedReason || title === selected.canonical_title || missing > 0} onClick={() => previewChange.mutate()}><Check size={16} />预览修改</button>}</Dialog></> : <Empty>选择番剧查看季集详情和纠正</Empty>}</aside></div>}
  </Page>
}
...
function RulesPanel({ active = true }: { active?: boolean }) {
  ...
  return <><p className="risk-warning">仅供熟悉 AutoAnime 规则格式的用户使用。普通用户无需创建或修改规则 JSON。</p><div className="plan-layout editor-layout"><section className="surface"><form className="inline-form" onSubmit={event => { event.preventDefault(); createSet.mutate() }}><input aria-label="规则集名称" value={name} onChange={event => setName(event.target.value)} placeholder="例如：默认别名" /><button className="primary" disabled={!name}><Plus size={16} />新建规则集</button></form><DataTable items={query.data?.items || []} columns={['id', 'name', 'active_revision_id', 'updated_at']} onRow={item => setSelectedId(item.id)} selectedId={selected?.id} /></section><aside className="inspector">{selected ? <><h2>{selected.name}</h2><label>规则 JSON<textarea className="code-editor" aria-label="规则 JSON" value={document} onChange={event => setDocument(event.target.value)} /></label>{error ? <div className="form-error">{error}</div> : null}<button className="secondary full" onClick={() => createRevision.mutate()}><Save size={16} />保存草稿</button><div className="revision-list">{selected.revisions?.map((revision: Item) => <div key={revision.id}><strong>修订 {revision.revision}</strong><Status value={revision.status} /><div className="row-actions">{revision.status === 'draft' ? <button className="text-button" onClick={() => validate.mutate(revision.id)}>校验</button> : null}{revision.status === 'validated' ? <button className="text-button" onClick={() => activate.mutate(revision.id)}>激活</button> : null}{revision.content_hash && revision.status !== 'active' ? <button className="text-button" onClick={() => rollback.mutate(revision.id)}>回退到此版</button> : null}</div></div>)}</div>{latest ? <pre className="evidence">{JSON.stringify(latest.document, null, 2)}</pre> : <Empty>尚无修订</Empty>}</> : <Empty>先创建规则集</Empty>}</aside></div></>
}
...
function OperationsPanel({ active = true }: { active?: boolean }) {
  ...
  return <div className="plan-layout review-layout">{opError ? <div className="form-error">{opError}</div> : null}<section className="surface"><DataTable items={query.data.items} columns={['id', 'kind', 'status', 'created_at', 'finished_at']} onRow={setSelected} selectedId={selected?.id} action={item => ['execute', 'correction'].includes(item.kind) && item.status === 'completed' ? <button className="text-button danger" onClick={() => requestRollback(item.id)}><RotateCcw size={14} />回滚</button> : null} /></section><aside className="inspector">{detail.data ? <><h2>批次 #{detail.data.id}</h2><Status value={detail.data.status} /><pre className="evidence">{JSON.stringify(detail.data.summary, null, 2)}</pre><div className="event-list">{detail.data.items?.map((item: Item) => <div key={item.id}><strong>{ACTION_LABELS[String(item.action)] || item.action} · {item.status === 'completed' ? '已完成' : item.status === 'failed' ? '失败' : item.status === 'pending' ? '待定' : item.status}</strong><span>{item.source_path} → {item.destination_path}</span></div>)}</div></> : <Empty>选择批次查看文件摘要</Empty>}</aside></div>
}
...
function displayValue(column: string, value: unknown) {
  ...
  if (column === 'review_type') return REVIEW_TYPE_LABELS[String(value)] || String(value ?? '—')
  if (column === 'current_stage') return STAGE_LABELS[String(value)] || String(value ?? '—')
