...
export function LibraryPage() {
  ...
  return <Page title="资料库" description="查看番剧、季度、文件位置、识别证据和附加元数据">{!query.data?.items.length ? <Empty title="资料库还是空的" description="批准并完成一次整理后，番剧会显示在这里。" cta={{ label: '配置目录', to: '/scan' }} /> : <div className="plan-layout"><section className="surface"><DataTable items={query.data.items} columns={['canonical_title', 'status', 'revision', 'updated_at']} onRow={item => { setSelected(item); setTitle(item.canonical_title); setPreview(null) }} selectedId={selected?.id} /></section><aside className="inspector">{selected ? <>{metadata?.poster_url ? <img className="poster" src={metadata.poster_url} alt={`${selected.canonical_title} 海报`} /> : null}<h2>{selected.canonical_title}</h2><p className="muted-copy">{metadata?.synopsis || '暂无简介；元数据不可用不会影响文件整理。'}</p><dl><dt>放送状态</dt><dd>{metadata?.broadcast_status || '未知'}</dd><dt>季度数</dt><dd>{detail.data?.seasons?.length ?? 0}</dd><dt>修订</dt><dd>{selected.revision}</dd></dl><label>新规范标题<input aria-label="新规范标题" value={title} onChange={event => setTitle(event.target.value)} /></label><label>修改原因<input aria-label="修改原因" value={reason} onChange={event => setReason(event.target.value)} /></label><label className="check-field"><input type="checkbox" checked={locked} onChange={event => setLocked(event.target.checked)} />锁定人工标题</label>{impact.data?.merge ? <div className="risk-warning">该标题已是番剧「{impact.data.target_show?.canonical_title}」。批准后将合并：移动 {moving} 个文件，同集自动留大删小（{discarding} 个）。</div> : moving + discarding > 0 ? <div className="risk-warning">批准后将移动 {moving} 个文件{discarding > 0 ? `，并删小留大 ${discarding} 个` : ''}到新文件夹。</div> : null}{preview ? <div className="change-preview"><strong>修改预览</strong><span>{String(preview.old_values?.canonical_title)} → {String(preview.new_values?.canonical_title)}</span><button className="primary full" onClick={() => approve.mutate()}><ShieldCheck size={16} />批准修改</button></div> : <button className="secondary full" disabled={!title || !reason || title === selected.canonical_title} onClick={() => previewChange.mutate()}><Check size={16} />预览修改</button>}</> : <Empty>选择番剧查看详情和纠正</Empty>}</aside></div>}</Page>
}
