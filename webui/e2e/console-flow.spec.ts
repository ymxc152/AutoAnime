...
test('login, configure, scan, approve, execute and rollback real file', async ({ page }) => {
  ...
  await page.getByRole('button', { name: '新建扫描方案' }).click()
  const createDialog = page.getByRole('dialog', { name: '新建扫描方案' })
  await createDialog.getByLabel('配置名称').fill('E2E 真实整理')
  await createDialog.getByLabel('下载源').selectOption({ label: source })
  await createDialog.getByLabel('媒体库').selectOption({ label: library })
  await createDialog.getByRole('button', { name: '创建扫描方案' }).click()
  await page.getByRole('button', { name: '编辑' }).click()
  const editDialog = page.getByRole('dialog', { name: '编辑扫描方案' })
  await editDialog.getByRole('button', { name: '更多选项' }).click()
  await editDialog.getByLabel('最低置信度').fill('90')
...
test('scan page can delete a scan profile and its roots', async ({ page }) => {
  ...
  await page.getByRole('button', { name: '新建扫描方案' }).click()
  const createDialog = page.getByRole('dialog', { name: '新建扫描方案' })
  await createDialog.getByLabel('配置名称').fill('待删除方案')
  await createDialog.getByLabel('下载源').selectOption({ label: source })
  await createDialog.getByLabel('媒体库').selectOption({ label: library })
  await createDialog.getByRole('button', { name: '创建扫描方案' }).click()
...
test('scan page edits a profile name with a visible edit state', async ({ page }) => {
  ...
  await page.getByRole('button', { name: '新建扫描方案' }).click()
  const createDialog = page.getByRole('dialog', { name: '新建扫描方案' })
  await createDialog.getByLabel('配置名称').fill('旧名称')
  await createDialog.getByLabel('下载源').selectOption({ label: source })
  await createDialog.getByLabel('媒体库').selectOption({ label: library })
  await createDialog.getByRole('button', { name: '创建扫描方案' }).click()
  ...
  const editor = page.getByRole('dialog', { name: '编辑扫描方案' })
  await expect(editor.getByText(/正在编辑扫描方案「旧名称」/)).toBeVisible()
  ...
  await editor.getByLabel('配置名称').fill('新名称')
...
conn.commit()
  ...
  await page.getByRole('button', { name: '问助手' }).click()
