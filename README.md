...
- **三级识别管线**：L1 本地规则（确定性，零网络）→ L2 识别记忆（确认过的命名模式直接命中）→ L3 LLM 识别（可选）+ 参考源消歧，仲裁器（arbiter）按置信度裁决：HIGH 自动归档 / MEDIUM 待人工确认 / LOW 进待确认队列。
...
- **1058 个后端离线测试**（`uv run pytest -q`，全程不触网）+ **67 个前端测试**（`cd frontend && npm test`），全部通过。
- **三轮真实数据验收**：第一轮修复 4 项；第二、三轮累计修复 10 项，其中包括两个重大缺陷（episode id 与集号混用、洗版目标位覆盖）。
