...
# NOTE: this file is BOTH a .bat and an embedded PowerShell script (polyglot).
# The batch section above ends at "exit /b 0"; PowerShell reads this same file,
# cuts everything before the marker and runs the rest as a script block.
# Keep the PowerShell section free of block comments; use # line comments
# only (a block-comment terminator would confuse the extraction).
param(
...
# $PSScriptRoot is empty for an embedded script block; the batch has already
# cd'd into the project root, so derive it from the current location.
